...
class GLaDOS {
  ...
  async getUserInfo(userId, token) {
    ...
    try {
      const response = await axios.get(`${this.userInfoUrl}${userId}/`, {
        headers,
      });
      return response.data;
    } catch (error) {
  ...
  async holdCoins(token) {
    ...
    try {
      const response = await axios.post(this.holdCoinsUrl, payload, {
        headers,
      });
  ...
  async swipeCoin(token) {
    ...
    try {
      ...
      if (getResponse.data.success) {
        ...
        const postResponse = await axios.post(this.swipeCoinUrl, payload, {
          headers,
        });
  ...
  async getDailyTasks(token) {
    ...
    try {
      const response = await axios.get(`${this.tasksUrl}?is_daily=false`, {
        headers,
      });
  ...
  async completeTask(token, task) {
    ...
    try {
      ...
      if (response.data.is_completed) {
        this.log(
          `Làm nhiệm vụ ${task.id}: ${task.title.yellow} .. trạng thái: thành công`
            .green
        );
      } else {
  ...
  async handleDurovTask(token) {
    try {
      const getResponse = await axios.get(this.durovUrl, {
      ...
      if (getResult.detail && getResult.detail.blocked_until) {
        ...
        this.log(
          `Tìm câu đố Durov không thành công, cần mời thêm ${getResult.detail.need_invites} bạn hoặc chờ đến ${blockedTime}`
            .yellow
        );
        return;
      }
      ...
      if (today.hasSame(payloadDate, "day")) {
        ...
        const postResponse = await axios.post(this.durovUrl, payload, {
          headers: this.headers(token),
        });
        ...
        this.log(
          "Chưa có combo Durov ngày mới, cần réo tên @hung96 để cập nhật combo"
            .yellow
        );
      } else {
        this.log(
          "Payload date is in the future. Please check the date format.".yellow
        );
      }
    } catch (error) {
      if (
        ...
        if (detail.blocked_until) {
          const blockedTime = DateTime.fromSeconds(detail.blocked_until)
            .setZone("system")
            .toLocaleString(DateTime.DATETIME_MED);
          this.log(
            `Tìm câu đố Durov không thành công, cần mời thêm ${detail.need_invites} bạn hoặc chờ đến ${blockedTime}`
              .yellow
          );
        } else {
    ...
    while (true) {
      for (let i = 0; i < data.length; i++) {
        ...
        if (authResult) {
          ...
          if (userInfo) {
            this.log(
              `Số sao đang có: ${userInfo.rating.toString().white}`.green
            );
          }
          ...
          if (visitResult) {
            if (visitResult.is_increased) {
              ...
              this.log(
                `Đã điểm danh trước đó. Streak hiện tại: ${visitResult.streak}`
                  .yellow
              );
            }
          }
          ...
          if (rouletteResult) {
            if (rouletteResult.rating_award > 0) {
              ...
              const blockedTime = DateTime.fromSeconds(
                ...
                .toLocaleString(DateTime.DATETIME_MED);
              this.log(
                `Spin không thành công, cần mời thêm ${rouletteResult.detail.need_invites} bạn hoặc chờ đến ${blockedTime}`
                  .yellow
              );
            } else {
          ...
          if (holdCoinsResult.detail && holdCoinsResult.detail.blocked_until) {
            const blockedTime = DateTime.fromSeconds(
              ...
              .toLocaleString(DateTime.DATETIME_MED);
            this.log(
              `HOLD coin không thành công, cần mời thêm ${holdCoinsResult.detail.need_invites} bạn hoặc chờ đến ${blockedTime}`
                .yellow
            );
          }
          ...
          if (swipeCoinResult.detail && swipeCoinResult.detail.blocked_until) {
            const blockedTime = DateTime.fromSeconds(
              ...
              .toLocaleString(DateTime.DATETIME_MED);
            this.log(
              `Swipe coin không thành công, cần mời thêm ${swipeCoinResult.detail.need_invites} bạn hoặc chờ đến ${blockedTime}`
                .yellow
            );
          }
